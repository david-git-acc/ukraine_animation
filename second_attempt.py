...
textfile = open("ukr_cities3.txt", "r+")

# Stores all of the above data in a dictionary (called towns instead of cities)
towns_dict = {}

# Each line is for a city, it's a CSV text file
for line in textfile:
    
    # This takes each line and gives a list of info, with the list elements being the above data in order
    info = line[:len(line)-1:].replace(":",",").split(",")
    
    
    town = info[0]
    lat = float(info[1])
    lon = float(info[2])
    score = int(info[3]) # Scalar multiplier 
    population = int(info[4])
    

    # Storing all the important information about each city in the towns_dict
    towns_dict.update({ town : (lat, lon, score, population) })
